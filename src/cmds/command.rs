...
use crate::tools::error::Error::{self, NoRemote};
...
pub async fn get_command(args: &[String]) -> Result<Box<dyn Command>, Error> {
...
pub async fn start(target: &Path) -> Result<(), Error> {
    let home_dir = FileSystem::home_backup_dir(target);
    let target_dir = target.join(BURST_DIRECTORY);
    let mut exist = tokio::fs::try_exists(&home_dir).await.map_err(|err| Error::IoError(IoError::from(&home_dir, err)))?;
    if !exist {
        return Err(Error::InvalidBackupDirectory());
    }
    ...
    exist = tokio::fs::try_exists(&remote_cfg).await.map_err(|err| Error::IoError(IoError::from(&remote_cfg, err)))?;
    if !exist {
        return Err(NoRemote());
    }
    exist = tokio::fs::try_exists(&remote_db).await.map_err(|err| Error::IoError(IoError::from(&remote_db, err)))?;
    if !exist {
        return Err(NoRemote());
    }

    exist = tokio::fs::try_exists(&local_cfg).await.map_err(|err| Error::IoError(IoError::from(&local_cfg, err)))?;
    if !exist {
        tokio::fs::copy(&remote_cfg, &local_cfg)
            .await
            .map_err(|err| Error::IoError(IoError::from_str("Cannot copy configuration", err)))?;
    }
    exist = tokio::fs::try_exists(&local_db).await.map_err(|err| Error::IoError(IoError::from(&local_db, err)))?;
    if !exist {
        tokio::fs::copy(&remote_db, &local_db).await.map_err(|err| Error::IoError(IoError::from_str("Cannot copy metadata", err)))?;
    }
    Ok(())
}

pub async fn stop(target: &Path) -> Result<(), Error> {
    let home_dir = FileSystem::home_backup_dir(target);
    let target_dir = target.join(BURST_DIRECTORY);
    ...
    let remote_cfg = target_dir.join(BURST_CONFIG_FILE);
    let remote_db = target_dir.join(BURST_METADATA_FILE);
    let exist = tokio::fs::try_exists(&target_dir).await.map_err(|err| Error::IoError(IoError::from(&target_dir, err)))?;
    if !exist {
        return Ok(());
    }

    tokio::fs::copy(&local_cfg, &remote_cfg).await.map_err(|err| Error::IoError(IoError::from_str("Cannot copy configuration", err)))?;
    tokio::fs::copy(&local_db, &remote_db).await.map_err(|err| Error::IoError(IoError::from_str("Cannot copy metadata", err)))?;
    Ok(())
}
